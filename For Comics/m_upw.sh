...
if ask_confirmation; then
    ...
    for ((i = $numini; i <= $numfin; i++)); do
        ...
        find . -type d -name "Chapter ${number}*" -print0 | while read -d '' -r dir; do
            ...
            magick chapter.webp -stroke 'rgba(0,0,0,0)' -strokewidth 3 -font "$HOME/.local/bin/OldLondon.ttf" -pointsize 150 -gravity center -fill 'rgba(245, 245, 245,1)' -annotate +0-880 "$a" "${dir}/000.webp"
        done
        let a=a+1
    done
