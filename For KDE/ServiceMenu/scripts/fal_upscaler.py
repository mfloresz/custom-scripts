...
def notify(message):
    try:
        subprocess.run(['notify-send', 'Fal.ai Clarity Upscaler', message])
...
def upscale_image(input_image_path):
    try:
        logging.info(f"Iniciando proceso con imagen: {input_image_path}")
        notify("Iniciando proceso de mejora de imagen...")
        ...
        output_image_path = os.path.splitext(input_image_path)[0] + "-upscaled.png"
        ...
        error_msg = f"Error general en upscale_image: {str(e)}"
        logging.error(error_msg)
        notify(error_msg)
        raise
...
if __name__ == "__main__":
    try:
        ...
        if len(sys.argv) != 2:
            error_msg = "Uso: python3 fal_clarity_upscaler.py <ruta_de_la_imagen>"
            logging.error(error_msg)
            notify(error_msg)
            sys.exit(1)
